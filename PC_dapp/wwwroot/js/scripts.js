  ...
  const setMany = (selector, values) => {
    ...
    values.forEach((value, index) => {
      if (els[index]) els[index].textContent = value;
                                    ...
                                    setText('.sidebar-title', '☰ Danh mục sản phẩm');
                                    setMany('.cat-item', ['⚡ CPU - Bộ vi xử lý', '🔩 Mainboard', '🖼️ VGA - Card đồ họa', '💾 RAM - Bộ nhớ trong', '🌀 SSD - Ổ cứng thể rắn', '🖥️ Màn hình máy tính', '🏗️ Case - Thùng máy', '🔌 PSU - Nguồn máy tính', '❄️ Tản nhiệt CPU', '🖱️ Chuột - Bàn phím', '🎧 Tai nghe - Loa', '🖨️ Máy in - Máy scan']);
  ...
  const setMany = (selector, values) => {
    ...
    values.forEach((value, index) => {
      if (els[index]) els[index].textContent = value;
                                    ...
                                    setMany('.qcat-name', ['CPU &\nMainboard', 'Card\nDo Hoa', 'RAM\n& SSD', 'Man\nHinh', 'Case\n& PSU', 'Tan\nNhiet', 'Ngoai\nVi', 'PC\nGaming']);
                                    setText('.sidebar-title', 'Danh muc san pham');
                                    setMany('.cat-item', ['CPU - Bo vi xu ly', 'Mainboard', 'VGA - Card do hoa', 'RAM - Bo nho trong', 'SSD - O cung the ran', 'Man hinh may tinh', 'Case - Thung may', 'PSU - Nguon may tinh', 'Tan nhiet CPU', 'Chuot - Ban phim', 'Tai nghe - Loa', 'May in - May scan']);
